"""Train a larger AMST-Net on 3-channel log-scale input with strong GBDT prior.

Architecture bump vs the strong-prior baseline:
  d_trans  128 -> 256
  d_mamba   64 -> 128
  n_trans_layers 2 -> 3
  n_mamba_layers 2 -> 3
  n_heads    4 -> 8
Goal: see if capacity can learn the patterns all strong models currently miss.
"""
import os
import sys
import time
...
# Redirect stdout/stderr to a log file so background progress is visible
_log_path = os.path.join(OUTPUT_DIR, 'amst_3ch_large_strong_prior.log')
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_large_strong_prior_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_large_strong_prior_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_large_strong_prior_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch large strong prior ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_large_strong_prior_oof.npz'),
    oof_amst_3ch_large_strong_prior=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_large_strong_prior_oof.npz")}')
