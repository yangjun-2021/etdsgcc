"""Train Informer with the fast config used in the archive script."""
import os
import sys
import time
...
# Fast config matching the archive script that produced F1=0.8485
CONFIG = {
    'd_model': 64,
    'n_heads': 4,
    'num_layers': 2,
...
for fi, (ti, vi) in enumerate(skf.split(X_seq, flags)):
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'informer_fast_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'informer_fast_oof.npz'),
    oof_informer_fast=oof,
    flags=flags,
)
print(f'\nSaved to {os.path.join(OUTPUT_DIR, "informer_fast_oof.npz")}')
