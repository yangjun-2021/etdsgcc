"""Screen PatchTransformer on raw 3ch with time-series augmentation."""
import os
import sys
import time
...
# Apply TSA to training set only
tsa = TSAugment(methods=['jitter', 'scale'], prob=0.5, severity=0.1, seed=SEED)
X_train_aug, y_train_aug = tsa.fit_transform(X_seq[train_idx], flags[train_idx], n_aug_per_sample=1)
prior_train_aug = np.tile(oof_prior[train_idx], 2)
print(f'After TSA: X_train_aug {X_train_aug.shape}, theft rate {y_train_aug.mean()*100:.2f}%', flush=True)
