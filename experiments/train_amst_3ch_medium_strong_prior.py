"""Train a medium-capacity AMST-Net on 3-channel input with strong GBDT prior.

Capacity sits between the strong-prior baseline (d_trans=128/d_mamba=64,
2 layers) and the oversized large model.  Expected params ~1.2M and
~2x the runtime of the baseline, which is feasible on the laptop GPU.
"""
import os
import sys
import time
...
LOG_PATH = os.path.join(OUTPUT_DIR, 'amst_3ch_medium_strong_prior.log')
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_medium_strong_prior_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_medium_strong_prior_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_medium_strong_prior_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch medium strong prior ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_medium_strong_prior_oof.npz'),
    oof_amst_3ch_medium_strong_prior=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_medium_strong_prior_oof.npz")}')
