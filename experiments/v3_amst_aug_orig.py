"""V3 voter: AMST-Net 3ch + mixed theft-attack augmentation, ORIGINAL labels, NO prior.

Untainted voter for the v3 consensus label cleaning. Augmentation is fold-aware
(train indices only): 1x PeerJ anomalies + 1x theft-attack anomalies, both
generated from the train fold. No prior feature is used.

Usage:
    conda run -n ml python experiments/v3_amst_aug_orig.py
...
flags = cl['y_orig'].astype(int)

out_prefix = 'v3voter_amst_aug'
...
for fi, (ti, vi) in enumerate(splits):
    ...
    n_syn = int(y_train.sum())  # 1x train-fold positives per augmenter
    ...
    train_loader = trainer._build_loaders(X_train_aug, y_train_aug, prior=None, shuffle=True)
    val_loader = trainer._build_loaders(X_val, y_val, prior=None, shuffle=False)
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'{out_prefix}_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
...
print(f'\n=== Overall V3-voter AMST 3ch mixed-aug (original labels, no prior) ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, f'{out_prefix}_oof.npz'),
    oof_v3voter_amst_aug=oof,
    y_orig=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, f"{out_prefix}_oof.npz")}')
