"""Train diverse GBDT experts (LGB+XGB+Cat per variant) and ensemble."""
import os, sys, time
...
def train_triple_ensemble(X, y, name, skf, lgb_cfg, xgb_cfg, cat_cfg):
    ...
    print(f'  {name:25s}: F1={best_f1:.4f} AUC={roc_auc_score(y,ens):.4f} weights={best_w}')
    return ens
...
def main():
    ...
    oofs = {}
    print('\nTraining triple-ensemble variants...')
    ...
    print(f'F1={best_f1:.4f}, Rec={recall_score(y,pred):.4f}, '
          ...
          f'AUC={roc_auc_score(y,best_oof):.4f}, th={best_th:.3f}')
    print(f'TP={tp} FP={fp} FN={fn}')

    np.savez_compressed(
        os.path.join(OUTPUT_DIR, 'mega_gbdt_recall_v2_oof.npz'),
        oof_mega_gbdt_recall_v2=best_oof,
        flags=y,
