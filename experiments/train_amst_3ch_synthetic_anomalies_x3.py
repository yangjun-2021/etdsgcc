"""Train AMST-Net on raw 3ch input with PeerJ-style synthetic anomaly injection.

This is the full 5-fold OOF training intended to produce a strong new OOF signal
for the meta-learner. Synthetic anomalies are injected per-fold into the training
set only (no validation leakage).
"""
import os
import sys
import time
...
trainer_kwargs = dict(
    ...
    use_prior=True,
    # Medium-sized model (matches recall10 config but with synthetic anomalies)
    d_mamba=64,
    d_trans=128,
    d_freq=64,
    proj_dim=128,
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_x3_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    # Augment training fold only (synthetic anomalies happen inside trainer._augment)
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_x3_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch synthetic anomalies ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_x3_oof.npz'),
    oof_amst_3ch_synthetic_x3=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_x3_oof.npz")}')
