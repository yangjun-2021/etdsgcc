"""Train medium AMST with light TSA + AMP as a diversity run.

If the plain medium AMST reaches a wall, this variant adds cheap jitter/scale
augmentation (copies=1) to expose slightly different temporal patterns.
"""
import os
import sys
import time
...
LOG_PATH = os.path.join(OUTPUT_DIR, 'amst_3ch_medium_tsa_amp.log')
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_medium_tsa_amp_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_medium_tsa_amp_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_medium_tsa_amp_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch medium TSA+AMP ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_medium_tsa_amp_oof.npz'),
    oof_amst_3ch_medium_tsa_amp=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_medium_tsa_amp_oof.npz")}')
