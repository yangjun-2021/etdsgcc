"""Train AMST-Net raw 3ch with 3x PeerJ synthetic anomalies + stronger GBDT prior v2."""
import os
import sys
import time
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_x3_sp_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_x3_sp_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_x3_sp_oof.npz'),
    oof_amst_3ch_synthetic_x3_sp=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_x3_sp_oof.npz")}')
