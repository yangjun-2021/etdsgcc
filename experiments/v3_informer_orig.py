"""V3 voter: Informer on 3ch input, ORIGINAL labels, NO prior feature (contingency voter).

Usage:
    conda run -n ml python experiments/v3_informer_orig.py
...
for fi, (ti, vi) in enumerate(splits):
    ...
    probs = predict_informer(model, X_seq[vi], None, device='cuda')
...
print(f'\n=== Overall V3-voter Informer (original labels, no prior) ===')
