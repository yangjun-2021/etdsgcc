"""V3 voter: AMST-Net 3ch + co-teaching, ORIGINAL labels, NO prior feature.

Untainted voter for the v3 consensus label cleaning: use_prior=False so no
cleaned-label-derived prior enters the model.

Usage:
    conda run -n ml python experiments/v3_amst_ct_orig.py
...
out_prefix = 'v3voter_amst_ct'

trainer_kwargs = dict(
    dataset='sgcc',
    use_diffaug=False,
    use_supcon=False,
    use_coteaching=True,        # label-noise robust co-teaching
    forget_rate=0.15,
    num_gradual=10,
    use_prior=False,            # untainted: no cleaned-label-derived prior
    d_mamba=64,
    d_trans=128,
    d_freq=64,
    proj_dim=128,
...
print(f'\n=== Overall V3-voter AMST 3ch coteaching (original labels, no prior) ===')
