...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_oof.npz'),
    oof_amst_3ch_synthetic=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_oof.npz")}')
