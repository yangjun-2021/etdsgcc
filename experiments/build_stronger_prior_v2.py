"""Build a stronger GBDT prior by including more internal OOF signals."""
import os
import sys
...
# Filter to OOFs with AUC > 0.965 (more inclusive than original 0.98)
strong_oofs = []
...
for fi, (ti, vi) in enumerate(skf.split(X, y)):
    ...
    m = lgb.LGBMClassifier(
        ...
        scale_pos_weight=pw * 1.5,  # slightly recall-oriented
...
print(f'\nStronger prior v2: F1={f1_score(y, pred):.4f}, Rec={recall_score(y, pred):.4f}, '
      f'Prec={precision_score(y, pred, zero_division=0):.4f}, AUC={roc_auc_score(y, prior):.4f}, th={best_th:.3f}')

np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'stronger_gbdt_prior_v2.npz'),
    prior=prior,
    flags=y,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "stronger_gbdt_prior_v2.npz")}')
