"""Train recall-oriented PatchTransformer on raw 3ch with 5-fold CV."""
import os
import sys
import time
...
LOG_PATH = os.path.join(OUTPUT_DIR, 'patch_transformer_raw_3ch_recall.log')
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'patch_transformer_raw_3ch_recall_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'patch_transformer_raw_3ch_recall_fold{fi}.npz'),
        oof=oof[vi],
        vi=vi,
        ti=ti,
    )
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'patch_transformer_raw_3ch_recall_fold{fi}.pt'))
...
print(f'\n=== Overall PatchTransformer raw 3ch recall ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'patch_transformer_raw_3ch_recall_oof.npz'),
    oof_patch_transformer_raw_3ch_recall=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "patch_transformer_raw_3ch_recall_oof.npz")}')
