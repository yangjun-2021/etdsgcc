"""Fast single-split screen of PatchTransformer on log1p 5ch input."""
import os
import sys
import time
...
X_seq = pre['X_seq']
flags = pre['flags']
print(f'X_seq shape: {X_seq.shape}, theft rate: {flags.mean()*100:.2f}%', flush=True)
...
val_probs = predict_patch_transformer(model, X_seq[val_idx], oof_proba_a[val_idx], device='cuda')
