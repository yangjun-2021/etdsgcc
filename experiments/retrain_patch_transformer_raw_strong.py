"""Train stronger PatchTransformer on raw labels with 5-fold CV."""
import os
import sys
import time
...
LOG_PATH = os.path.join(OUTPUT_DIR, 'patch_transformer_raw_strong.log')
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'patch_transformer_raw_strong_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    model = PatchTransformerClassifier(
        ...
        patch_len=30, stride=15, d_model=128, n_layers=3, n_heads=4,
    ...
    criterion = RecallOrientedFocalLoss(alpha=0.75, gamma=2.0, recall_weight=10.0).to('cuda')
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'patch_transformer_raw_strong_fold{fi}.npz'),
        oof=oof[vi],
        vi=vi,
        ti=ti,
    )
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'patch_transformer_raw_strong_fold{fi}.pt'))
...
print(f'\n=== Overall PatchTransformer Raw Strong ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'patch_transformer_raw_strong_oof.npz'),
    oof_patch_transformer_raw_strong=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "patch_transformer_raw_strong_oof.npz")}')
