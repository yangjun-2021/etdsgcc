"""Train SupCon TCN on raw-scale 3ch input with 5-fold CV."""
import os
import sys
import time
...
LOG_PATH = os.path.join(OUTPUT_DIR, 'supcon_raw_3ch.log')
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'supcon_raw_3ch_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    probs = predict_supcon(model, X_seq[vi], oof_prior[vi], device='cuda', batch_size=512)
    oof[vi] = np.nan_to_num(probs, nan=0.5)

    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'supcon_raw_3ch_fold{fi}.npz'),
        oof=oof[vi],
        vi=vi,
        ti=ti,
    )
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'supcon_raw_3ch_fold{fi}.pt'))
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'supcon_raw_3ch_oof.npz'),
    oof_supcon_raw_3ch=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "supcon_raw_3ch_oof.npz")}')
