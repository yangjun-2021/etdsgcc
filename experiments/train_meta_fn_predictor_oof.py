"""Train an XGB model to predict current meta's false negatives, and save OOF.

The target is binary: 1 if the current best meta prediction is a false negative,
0 otherwise.  The OOF probabilities are produced by 5-fold CV and can be fed
back into the meta-learner as an additional error-correction signal.
"""
import os
import sys
import numpy as np
...
# Build FN target from current best meta threshold
...
fn_target = ((meta_pred == 0) & (flags == 1)).astype(int)
print(f'Best meta th={best_th:.3f} F1={best_f1:.4f}, FN target rate={fn_target.mean()*100:.2f}%')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'meta_fn_predictor_oof.npz'),
    oof_meta_fn_predictor=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "meta_fn_predictor_oof.npz")}')
