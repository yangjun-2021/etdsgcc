"""Train larger Informer with strong prior and full precision."""
import os
import sys
import time
...
for fi, (ti, vi) in enumerate(skf.split(X_seq, flags)):
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'informer_large_strong_prior_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'informer_large_strong_prior_oof.npz'),
    oof_informer_large_strong_prior=oof,
    flags=flags,
)
print(f'\nSaved to {os.path.join(OUTPUT_DIR, "informer_large_strong_prior_oof.npz")}')
