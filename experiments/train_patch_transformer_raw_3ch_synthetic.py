"""Train Patch Transformer raw 3ch with PeerJ-style synthetic anomaly injection."""
import os
import sys
import time
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'patch_transformer_raw_3ch_synthetic_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'patch_transformer_raw_3ch_synthetic_fold{fi}.npz'),
        oof=oof[vi],
        vi=vi,
        ti=ti,
    )
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'patch_transformer_raw_3ch_synthetic_fold{fi}.pt'))
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'patch_transformer_raw_3ch_synthetic_oof.npz'),
    oof_patch_transformer_raw_3ch_synthetic=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "patch_transformer_raw_3ch_synthetic_oof.npz")}')
