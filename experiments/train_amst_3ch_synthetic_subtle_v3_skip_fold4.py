"""Fast AMST raw 3ch: PeerJ + theft (with subtle types) + prior v3 + label smoothing.

Targets low-usage subtle theft cases that the current best model misses.
"""
import os
import sys
import time
...
prior_data = np.load(os.path.join(OUTPUT_DIR, 'stronger_gbdt_prior_v3.npz'))
oof_prior = prior_data['prior']

# PeerJ 1x + theft 1x (mix of strong + subtle types) = 2x training-fold positives
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_subtle_v3_skip_fold4_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    if fi in completed_folds:
        ...
        continue
    if fi == 3:
        print(f'Fold {fi+1}/{N_FOLDS} skipped due to persistent training hang')
        continue
    ...
    # Theft anomalies: 0.5x strong + 0.5x subtle, total 1x
    n_strong = N_SYN // 2
    n_subtle = N_SYN - n_strong

    theft_strong = TheftSyntheticAnomalyAugmenter(
        seed=SEED + fi + 100, n_synthetic=n_strong,
        anomaly_types=['scale_global', 'zero_interval', 'scale_local',
                       'subtract_random', 'subtract_mean', 'reverse_daily'])
    X_th_s, y_th_s = theft_strong.fit_transform(X_train, y_train)

    theft_subtle = TheftSyntheticAnomalyAugmenter(
        seed=SEED + fi + 200, n_synthetic=n_subtle,
        anomaly_types=['slight_scale_global', 'slight_scale_local', 'cap_max',
                       'reduce_night', 'flatten'])
    X_th_sub, y_th_sub = theft_subtle.fit_transform(X_train, y_train)

    X_train_aug = np.concatenate([X_train, X_pj, X_th_s, X_th_sub], axis=0)
    y_train_aug = np.concatenate([y_train, y_pj, y_th_s, y_th_sub], axis=0)
    prior_train_aug = np.concatenate([
        prior_train,
        np.full(len(y_pj) + len(y_th_s) + len(y_th_sub), prior_train.mean(), dtype=np.float32)
    ])
    ...
    trainer = AMSTTrainer(
        ...
        epochs=25, batch_size=128, lr=1e-4, patience=10, recall_weight=10.0, use_amp=True, label_smoothing=0.1,
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_subtle_v3_skip_fold4_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_subtle_v3_skip_fold4_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch subtle mixed synthetic anomalies + prior v3 ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_subtle_v3_skip_fold4_oof.npz'),
    oof_amst_3ch_synthetic_subtle_v3_skip_fold4=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_subtle_v3_skip_fold4_oof.npz")}')
