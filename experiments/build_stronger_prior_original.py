"""Build a strong GBDT prior on ORIGINAL SGCC labels.

This mirrors build_stronger_prior_v3.py but uses y_orig instead of cleaned flags.
The resulting prior can be used both as a standalone OOF and as a prior for
retraining deep experts on original labels.
"""
import os
import sys
...
if not os.path.exists(pre_path):
    raise FileNotFoundError(f'{pre_path} not found. Run experiments/retrain_expert_a_original.py first.')
...
for f, k in [
    ...
    # Original-label robust OOFs
    ('coteaching_original_oof.npz', 'oof_coteaching_original'),
    ('amst_3ch_original_coteaching_oof.npz', 'oof_amst_3ch_original_coteaching'),
]:
    try:
...
# Filter to OOFs with AUC > 0.95 on original labels
strong_oofs = []
...
print(f'\nStronger prior ORIGINAL: F1={f1_score(y, pred):.4f}, Rec={recall_score(y, pred):.4f}, '
      f'Prec={precision_score(y, pred, zero_division=0):.4f}, AUC={roc_auc_score(y, prior):.4f}, th={best_th:.3f}')

np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'stronger_gbdt_prior_original.npz'),
    prior=prior,
    y_orig=y,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "stronger_gbdt_prior_original.npz")}')
