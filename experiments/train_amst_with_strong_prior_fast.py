"""Train AMST-Net fold-by-fold with strong prior, using internal methods for speed."""
import os
import sys
import time
...
for fi, (ti, vi) in enumerate(splits):
    ...
    # Apply TSA/Mixup if enabled (currently disabled)
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_strong_prior_fold{fi}.pt'))
    del model
