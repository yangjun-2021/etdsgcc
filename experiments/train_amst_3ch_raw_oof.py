"""Train AMST-Net on raw-scale 3-channel SGCC input with strong GBDT prior.

Uses the same compact config as the successful screen, and supports fold-level
resumption so a long run can be restarted safely.
"""
import os
import sys
import time
...
LOG_PATH = os.path.join(OUTPUT_DIR, 'amst_3ch_raw.log')
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_raw_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_raw_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_raw_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_raw_oof.npz'),
    oof_amst_3ch_raw=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_raw_oof.npz")}')
