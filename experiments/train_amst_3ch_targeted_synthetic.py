"""Train AMST-Net raw 3ch with targeted synthetic anomalies (FN-driven v2).

Targets the hard-FN patterns identified by error analysis:
  - high missing-value ratio
  - long zero/flatline segments
  - globally reduced consumption
"""
import os
import sys
import time
...
trainer_kwargs = dict(
    ...
    recall_weight=10.0,
    use_amp=True,
    use_synthetic_anomalies=False,  # targeted augmenter applied manually below
)

syn_kwargs = dict(
    anomaly_types=['long_zero', 'scale_down', 'flatline', 'mixed'],
    n_synthetic=int((flags == 1).sum()) // N_FOLDS,
    seed=SEED,
)
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_targeted_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    # Light TSA/mixup through trainer._augment (only cheap augmentations)
    X_train_aug, y_train_aug = trainer._augment(X_train, y_train)

    # Add targeted synthetic anomalies
    syn_aug = TargetedSyntheticAnomalyAugmenter(seed=SEED + fi, **syn_kwargs)
    X_syn, y_syn = syn_aug.fit_transform(X_train_aug, y_train_aug)
    X_train_aug = np.concatenate([X_train_aug, X_syn], axis=0)
    y_train_aug = np.concatenate([y_train_aug, y_syn], axis=0)
    print(f'  After targeted synthetic aug: {X_train_aug.shape}, theft rate={y_train_aug.mean()*100:.2f}%')
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_targeted_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_targeted_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch targeted synthetic anomalies ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_targeted_oof.npz'),
    oof_amst_3ch_synthetic_targeted=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_targeted_oof.npz")}')
