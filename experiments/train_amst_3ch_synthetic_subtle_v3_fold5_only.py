...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_subtle_v3_fold5_only_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    trainer = AMSTTrainer(
        ...
        epochs=1, batch_size=128, lr=1e-4, patience=10, recall_weight=10.0, use_amp=True, label_smoothing=0.1,
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_subtle_v3_fold5_only_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_subtle_v3_fold5_only_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_subtle_v3_fold5_only_oof.npz'),
    oof_amst_3ch_synthetic_subtle_v3_fold5_only=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_subtle_v3_fold5_only_oof.npz")}')
