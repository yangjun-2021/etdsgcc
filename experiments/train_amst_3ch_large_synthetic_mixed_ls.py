"""Large AMST raw 3ch: mixed PeerJ + theft-specific synthetic anomalies + stronger prior v2 + label smoothing."""
import os
import sys
import time
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_large_synthetic_mixed_ls_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    trainer = AMSTTrainer(
        ...
        n_mamba_layers=3, n_trans_layers=3, n_heads=8, dropout=0.2,
        epochs=20, batch_size=48, lr=8e-5, patience=10, recall_weight=10.0, use_amp=True, label_smoothing=0.1,
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_large_synthetic_mixed_ls_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_large_synthetic_mixed_ls_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch large mixed synthetic anomalies + LS ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_large_synthetic_mixed_ls_oof.npz'),
    oof_amst_3ch_large_synthetic_mixed_ls=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_large_synthetic_mixed_ls_oof.npz")}')
