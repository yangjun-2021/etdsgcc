"""Quick single-split screen of Informer on GPU."""
import os
import sys
import time
...
# Single stratified split for fast screening
train_idx, val_idx = train_test_split(
    np.arange(len(flags)), test_size=0.2, random_state=SEED, stratify=flags)

configs = [
    {'d_model': 128, 'n_heads': 8, 'num_layers': 2, 'dropout': 0.3, 'epochs': 20, 'batch_size': 128, 'lr': 3e-4},
    {'d_model': 128, 'n_heads': 8, 'num_layers': 3, 'dropout': 0.3, 'epochs': 20, 'batch_size': 128, 'lr': 3e-4},
    {'d_model': 256, 'n_heads': 8, 'num_layers': 2, 'dropout': 0.3, 'epochs': 20, 'batch_size': 64, 'lr': 3e-4},
]
...
for cfg in configs:
    ...
    model = train_informer(
        ...
        oof_prior=oof_proba_a[train_idx],
        device='cuda', seed=SEED, verbose=True,
        **cfg
    )
    val_probs = predict_informer(model, X_seq[val_idx], oof_proba_a[val_idx], device='cuda')
    ...
    print(f'  Val: F1={f1_score(flags[val_idx], pred):.4f}, Rec={recall_score(flags[val_idx], pred):.4f}, '
          f'Prec={precision_score(flags[val_idx], pred, zero_division=0):.4f}, AUC={roc_auc_score(flags[val_idx], val_probs):.4f}, '
          f'th={best_th:.3f}, time={elapsed:.1f}min')
    del model
