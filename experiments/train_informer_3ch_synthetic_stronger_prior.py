"""Train Informer 3ch with PeerJ synthetic anomalies + stronger GBDT prior v2."""
import os
import sys
import time
...
SYN_KWARGS = dict(
    ...
    n_synthetic=int((flags == 1).sum() * 4 / 5),  # 1x training-fold positives
)
...
for fi, (ti, vi) in enumerate(skf.split(X_seq, flags)):
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'informer_3ch_synthetic_sp_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'informer_3ch_synthetic_sp_oof.npz'),
    oof_informer_3ch_synthetic_sp=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "informer_3ch_synthetic_sp_oof.npz")}')
