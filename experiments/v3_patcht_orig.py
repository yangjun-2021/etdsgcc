"""V3 voter: Patch Transformer on raw-scale 3ch input, ORIGINAL labels, NO prior feature.

Untainted voter for the v3 consensus label cleaning: unlike
train_patch_transformer_raw_3ch_original.py, this script does NOT consume
strong_gbdt_prior.npz (which was trained on cleaned labels).

Usage:
    conda run -n ml python experiments/v3_patcht_orig.py
...
print(f'X_seq shape: {X_seq.shape}, original theft rate: {flags.mean()*100:.2f}%')

out_prefix = 'v3voter_patcht'
...
for fi, (ti, vi) in enumerate(splits):
    ...
    model = train_patch_transformer(
        ...
        device='cuda', seed=SEED + fi,
        **CONFIG
    )
    probs = predict_patch_transformer(model, X_seq[vi], None, device='cuda')
...
print(f'\n=== Overall V3-voter Patch Transformer (original labels, no prior) ===')
