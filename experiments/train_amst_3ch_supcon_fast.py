"""Train AMST-Net on 3-channel log-scale input with SupCon (no strong prior)."""
import os
import sys
import time
...
# SupCon config, no strong prior to encourage diverse representation
trainer_kwargs = dict(
    dataset='sgcc',
    use_diffaug=False,
    use_supcon=True,
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_supcon_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_supcon_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_supcon_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_supcon_oof.npz'),
    oof_amst_3ch_supcon=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_supcon_oof.npz")}')
