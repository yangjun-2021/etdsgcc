"""Fast AMST raw 3ch: 3x PeerJ synthetic anomalies + stronger prior v2 (15 epochs)."""
import os
import sys
import time
...
trainer_kwargs = dict(
    ...
    synthetic_anomaly_kwargs=dict(
        ...
        n_synthetic=2 * int((flags == 1).sum() * 4 / 5),
    ),
)
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_x3_sp_fast_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_x3_sp_fast_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_x3_sp_fast_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_x3_sp_fast_oof.npz'),
    oof_amst_3ch_synthetic_x3_sp_fast=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_x3_sp_fast_oof.npz")}')
