"""Train AMST-Net on 3-channel input with TSA + Mixup augmentation + strong prior."""
import os
import sys
import time
...
trainer_kwargs = dict(
    ...
    recall_weight=5.0,
    use_tsa=True,
    tsa_methods=['jitter', 'scale', 'timewarp'],
    tsa_prob=0.5,
    tsa_severity=0.1,
    tsa_copies=1,
    use_mixup=True,
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_tsa_mixup_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_tsa_mixup_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_tsa_mixup_fold{fi}.pt'))
    del model
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_tsa_mixup_oof.npz'),
    oof_amst_3ch_tsa_mixup=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_tsa_mixup_oof.npz")}')
