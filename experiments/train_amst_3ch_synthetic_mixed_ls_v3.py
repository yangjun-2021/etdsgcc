"""Fast AMST raw 3ch: mixed PeerJ + theft-specific synthetic anomalies + stronger prior v3 + label smoothing."""
import os
import sys
import time
...
for fi in range(N_FOLDS):
    fpath = os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_mixed_ls_v3_fold{fi}.npz')
...
for fi, (ti, vi) in enumerate(splits):
    ...
    np.savez_compressed(
        os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_mixed_ls_v3_fold{fi}.npz'),
        oof=val_proba,
        vi=vi,
        ti=ti,
    )
    print(f'  Saved fold {fi+1} OOF, time={(time.time()-t0)/60:.1f}min')
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f'amst_3ch_synthetic_mixed_ls_v3_fold{fi}.pt'))
    del model
...
print(f'\n=== Overall AMST 3ch mixed synthetic anomalies + prior v3 ===')
...
np.savez_compressed(
    os.path.join(OUTPUT_DIR, 'amst_3ch_synthetic_mixed_ls_v3_oof.npz'),
    oof_amst_3ch_synthetic_mixed_ls_v3=oof,
    flags=flags,
)
print(f'Saved to {os.path.join(OUTPUT_DIR, "amst_3ch_synthetic_mixed_ls_v3_oof.npz")}')
